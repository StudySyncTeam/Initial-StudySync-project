...
function renderNotesTracker(searchFilter = '') {
  ...
  container.querySelectorAll('.delete-note-cross').forEach(cross => {
    cross.addEventListener('click', function() {
      ...
      renderNotesTracker(document.getElementById('notes-search-field').value);
    });
  });
}
...
function renderFlashcardsGrid() {
  ...
  document.getElementById('trigger-add-subject').addEventListener('click', () => {
...
function renderScheduleTimetable() {
  document.querySelectorAll('.schedule-matrix-table tbody tr').forEach(row => {
    ...
    row.querySelectorAll('td:not(.hour-tag)').forEach((cell, idx) => {
      ...
      const dayKey = cell.getAttribute('data-day') || days[idx - 1];
...
function initActionEventListeners() {
  ...
  document.getElementById('subtab-tasks').addEventListener('click', function() {
    appState.sidebarRightTab = 'tasks';
    this.classList.add('active');
    document.getElementById('subtab-expenses').classList.remove('active');
    renderSidebarFeed();
  });
  document.getElementById('subtab-expenses').addEventListener('click', function() {
  ...
  document.getElementById('btn-create-task').addEventListener('click', () => {
    ...
    if (!titleInp.value.trim() || !dateInp.value.trim()) return;

    appState.tasks.push({ id: Date.now(), title: titleInp.value.trim(), deadline: dateInp.value.trim(), status: soonCheck.checked ? 'soon' : 'all' });
    titleInp.value = '';
    dateInp.value = '';
    soonCheck.checked = false;
    renderTaskTracker();
    renderSidebarFeed();
  });
  ...
  document.getElementById('task-filter-soon').addEventListener('click', function() {
    document.querySelectorAll('.filter-pill').forEach(p => p.classList.remove('active'));
  ...
  document.getElementById('notes-search-field').addEventListener('input', function() { renderNotesTracker(this.value); });
  ...
  document.getElementById('btn-trigger-add-note').addEventListener('click', () => {
